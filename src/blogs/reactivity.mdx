...
const lazyData = reactive(() => fetch(`https://data.mysite.io/users?n=${nthUser.value}`));
...
<Mermaid
  data={{
    edges: [["nthUser", "lazyData"]],
    nodes: [
      ["nthUser", { shape: "circle" }],
      ["lazyData", {}],
    ],
  }}
/>
